...
THREE.VRControls = function ( object, onError ) {
	...
	function gotVRDevices( devices ) {
		...
		if ( !vrInput && onError ) onError( 'VR input not available.' );
	}

	if ( navigator.getVRDisplays )
		navigator.getVRDisplays().then( gotVRDevices );
	else if ( navigator.getVRDevices )	// Deprecated API
	...
	this.resetPose = function () {
		if ( vrInput ) {
			if ( vrInput.resetPose !== undefined )
				vrInput.resetPose();
			else if ( vrInput.resetSensor !== undefined )	// Deprecated API
				vrInput.resetSensor();
			else if ( vrInput.zeroSensor !== undefined )	// Really deprecated API
				vrInput.zeroSensor();
		}
	};
